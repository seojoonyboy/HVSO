...
namespace UnityEngine.AssetGraph
{
    ...
    [CustomNode("Export/Export To Directory for Patcher", 100)]
    public class PatcherExporter : Node, Model.NodeDataImporter
    {
        ...
        [SerializeField] private SerializableMultiTargetString m_obbPath;
        ...
        [SerializeField] private SerializableMultiTargetString m_key;
        ...
        public override void Initialize(Model.NodeData data)
        {
            ...
            m_obbPath = new SerializableMultiTargetString();
            ...
            m_key = new SerializableMultiTargetString();
        ...
        {
            m_obbPath = new SerializableMultiTargetString(v1.ExporterObbPath);
            ...
            m_flattenDir = new SerializableMultiTargetInt();
            m_key = new SerializableMultiTargetString("5swdcsrBHoBYjDWi5VXN72");
        }

        public override Node Clone(Model.NodeData newData)
        {
            var newNode = new PatcherExporter();
            newNode.m_obbPath = new SerializableMultiTargetString(m_obbPath);
            ...
            newNode.m_key = new SerializableMultiTargetString(m_key);
        ...
        public override void OnInspectorGUI(NodeGUI node, AssetReferenceStreamManager streamManager, NodeGUIEditor editor, Action onValueChanged)
        {
            ...
            EditorGUILayout.HelpBox("Export To Directory: Export given files to output directory for Haegin Patcher.", MessageType.Info);
            ...
            using (new EditorGUILayout.VerticalScope(GUI.skin.box))
            {
                var disabledScope = editor.DrawOverrideTargetToggle(node, m_exportPath.ContainsValueOf(currentEditingGroup), (bool enabled) =>
                {
                    using (new RecordUndoScope("Remove Target Export Settings", node, true))
                    {
                        if (enabled)
                        {
                            m_obbPath[currentEditingGroup] = m_obbPath.DefaultValue;
                        ...
                        {
                            m_obbPath.Remove(currentEditingGroup);
                ...
                using (disabledScope)
                {
                    var newKey = EditorGUILayout.TextField("Encryption Key", m_key[BuildTargetGroup.Android]);
                    if (newKey != m_key[BuildTargetGroup.Android])
                    {
                        m_key[BuildTargetGroup.Android] = newKey;
                    }
                    ...
                    EditorGUILayout.LabelField("OBB Create Path:");

                    string newObbPath = null;

                    newObbPath = editor.DrawFolderSelector("", "Select Obb Folder",
                        m_obbPath[currentEditingGroup],
                        GetObbPath(m_obbPath[currentEditingGroup]),
                        (string folderSelected) =>
                        {
                            var projectPath = Directory.GetParent(Application.dataPath).ToString();
                    ...
                    if (newObbPath != m_obbPath[currentEditingGroup])
                    {
                        using (new RecordUndoScope("Change Obb Path", node, true))
                        {
                            m_obbPath[currentEditingGroup] = newObbPath;
                            onValueChanged();
                        }
                    }
            ...
            Action<Model.NodeData, string, float> progressFunc)
        {
            ...
            var obbPath = GetObbPath(m_obbPath[target]);
            var exportPath = GetExportPath(m_exportPath[target]);

            if (!Directory.Exists(obbPath))
            {
                Directory.CreateDirectory(obbPath);
            }
            obbPath = obbPath + "/main.obb";
            if (File.Exists(obbPath)) File.Delete(obbPath);

            Debug.Log("OBB File Path : " + obbPath);
            ...
            foreach (var ag in incoming)
            {
                ...
                foreach (var groupKey in ag.assetGroups.Keys)
                {
                    ...
                    if(inputSources == null || inputSources.Count <= 0) { continue; }
                    ...
                    if (source != null)
                    {
                        ...
                        var sourceDir = Directory.GetParent(source.importFrom).ToString();
                        var patcherDir = Directory.GetParent(destination).ToString();
#if MDEBUG
                        Debug.Log(source.importFrom);
                        Debug.Log("Patch Maker [" + sourceDir + "] => [" + patcherDir + "]");
#endif
                        if (!Directory.Exists(patcherDir))
                        {
                            Directory.CreateDirectory(patcherDir);
                        }
                        G.Network.PatchMaker patchMaker = new G.Network.PatchMaker();
                        patchMaker.Create(sourceDir, patcherDir, G.Util.ConvertEx.ToKey(m_key[BuildTargetGroup.Android]), obbPath);
                    }
                }
            }
            ...
            srcs.Add("base");
            srcs.Add("level1");
            G.Network.PatchMaker mergeMaker = new G.Network.PatchMaker();
            mergeMaker.PatchBinMerge(srcs, "/Users/Shared/Unity/HaeginModule_DotNet3/GeneratedPatchFiles/AssetBundles/iOS", "Merged.bin", G.Util.ConvertEx.ToKey(m_key[BuildTargetGroup.Android]));
            mergeMaker.PatchBinMerge(srcs, "/Users/Shared/Unity/HaeginModule_DotNet3/GeneratedPatchFiles/AssetBundles/Android", "Merged.bin", G.Util.ConvertEx.ToKey(m_key[BuildTargetGroup.Android]));
            mergeMaker.PatchBinMerge(srcs, "/Users/Shared/Unity/HaeginModule_DotNet3/GeneratedPatchFiles/AssetBundles/OSX", "Merged.bin", G.Util.ConvertEx.ToKey(m_key[BuildTargetGroup.Android]));
            mergeMaker.PatchBinMerge(srcs, "/Users/Shared/Unity/HaeginModule_DotNet3/GeneratedPatchFiles/AssetBundles/Windows", "Merged.bin", G.Util.ConvertEx.ToKey(m_key[BuildTargetGroup.Android]));
            mergeMaker.PatchBinMerge(srcs, "/Users/Shared/Unity/HaeginModule_DotNet3/GeneratedPatchFiles/AssetBundles/Linux", "Merged.bin", G.Util.ConvertEx.ToKey(m_key[BuildTargetGroup.Android]));
        ...
        private string GetObbPath(string path)
        {
            if (string.IsNullOrEmpty(path))
            {
                return Directory.GetParent(Application.dataPath).ToString();
