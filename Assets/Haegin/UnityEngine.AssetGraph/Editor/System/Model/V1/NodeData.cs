...
namespace AssetBundleGraph {
	...
	public class NodeData {
        ...
        //exporter settings
        private const string NODE_EXPORTER_OBB_PATH = "obbCreateTo";
		...
		[SerializeField] private SerializableMultiTargetString m_exporterObbPath;
        ...
        public SerializableMultiTargetString ExporterObbPath
        {
            get
            {
                ValidateAccess(
		...
		public void FromJsonDictionary(Dictionary<string, object> jsonData) {
			...
			case NodeKind.EXPORTER_GUI:
				{
                    m_exporterObbPath = new SerializableMultiTargetString(_SafeGet(jsonData, NODE_EXPORTER_OBB_PATH));
		...
		public NodeData(string name, NodeKind kind, float x, float y) {
			...
			case NodeKind.EXPORTER_GUI:
                m_exporterObbPath = new SerializableMultiTargetString();
		...
		public NodeData Duplicate (bool keepGuid = false) {
			...
			case NodeKind.EXPORTER_GUI:
                newData.m_exporterObbPath = new SerializableMultiTargetString(m_exporterObbPath);
		...
		public bool CompareIgnoreGUIChanges (NodeData rhs) {
			...
			case NodeKind.EXPORTER_GUI:
                if (m_exporterObbPath != rhs.m_exporterObbPath)
                {
                    LogUtility.Logger.LogFormat(LogType.Log, "{0} and {1} was different: {2}", Name, rhs.Name, "ExporterObbPath different");
                    return false;
                }
		...
		public Dictionary<string, object> ToJsonDictionary() {
			...
			case NodeKind.EXPORTER_GUI:
                nodeDict[NODE_EXPORTER_OBB_PATH] = m_exporterObbPath.ToJsonDictionary();
