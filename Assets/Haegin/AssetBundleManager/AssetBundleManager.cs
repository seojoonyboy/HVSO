...
namespace Haegin
{
    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        static public AssetBundleLoadManifestOperation Initialize(string manifestAssetBundleName, string subfolder = "")
        {
            BaseDownloadingURL = AssetBundleUtility.GetAssetBundlesPath() ;
            if(!string.IsNullOrEmpty(subfolder)) 
            {
                BaseDownloadingURL = System.IO.Path.Combine(BaseDownloadingURL, subfolder);
            }
        ...
        void Update()
        {
            ...
            {
                UnityWebRequest downloadReq = keyValue.Value;
                if (downloadReq.isNetworkError || downloadReq.isHttpError)
                {
                    m_DownloadingErrors.Add(keyValue.Key, string.Format("{0} network error", keyValue.Key));
                    keysToRemove.Add(keyValue.Key);
                    continue;
                }
                else if (downloadReq.isDone)
                {
                    AssetBundle bundle = DownloadHandlerAssetBundle.GetContent(downloadReq);
                    if (bundle == null)
                    {
            ...
            //{
            //             UnityWebRequest downloadReq = m_DownloadingWWWs[key];
